...
#RETURN_PAGE="http/return_13811311608_1488089999203610968_NqcirS3D"
...
doLog "submit amount request, num=$NUM, car_id=$CAR_ID, session=$SESSION"
...
RESP_URL=${RESP_URL//\\\//\/}
doLog "submit redirect request, url=$RESP_URL, car_id=$CAR_ID, session=$SESSION"
...
TRANS_DATA=$(php form_serialize.php $SUBMIT_PAGE "encode")
curl "https://transfer.moneymoremore.com/loan/loan.action" -H "Host: transfer.moneymoremore.com" -H "Accept-Encoding: gzip, deflate, br" -H "Accept-Language: zh-CN,zh;q=0.8" -H "User-Agent: Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/42.0.2311.152 Safari/537.36" -H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8" -H "Referer: $RESP_URL" -H "Connection: keep-alive" -H "Upgrade-Insecure-Requests: 1" -H "Cache-Control: max-age=0" -H "Origin: https://www.zecaifu.com" --data $TRANS_DATA --compressed -o $TRANSFER_PAGE

KEY=$(grep -Po '(?<=publicKey = ")[^"]+' $TRANSFER_PAGE) 
doLog "paypass verify request, pubkey=$KEY, car_id=$CAR_ID, session=$SESSION"
if [ "$KEY" == "" ];then
	doLog "pubick key is null, Exit"
	exit
fi
